...
def get_musdb(database_path):
    ...
    :return: dictionary with train and test keys, each containing list of samples, each sample containing all audio paths
    '''
    mus = musdb.DB(root=database_path, is_wav=True)
    ...
    for subset in ["train", "test"]:
        ...
        for track in (tracks):
            print(track.path)
            # Skip track if mixture is already written, assuming this track is done already
            track_path = track.path[:-12]
            print(track_path)
            if track_path=='MUSDB18_format_of_SNSD/train/.ipynb_checkpoints':
              continue
            if track_path=='MUSDB18_format_of_SNSD/test/.ipynb_checkpoints':
              continue  
...
def get_musdb_folds(root_path, version="HQ"):
    ...
    train_list = np.random.choice(train_val_list, 200, replace=False)
